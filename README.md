...
### 서버 — DataHub 전체
동적 게임 데이터(계정 정보, 보유 아이템, 장비 장착 상태, 상점 상태 등)를 처리하는 FastAPI 서버를 단독 설계·구현.

- REST API 엔드포인트 설계
- SQLAlchemy ORM으로 DB 스키마 정의
- admin 권한과 jwt 인증 시스템으로 보안 유지

### 클라이언트 — 데이터 관련 시스템 전담
데이터와 직접적인 연관을 갖는 시스템들에 대한 로직과 UI를 클라이언트에서 구현.

- DataAPI.cs, ApiModels.cs 등: 클라이언트에서 서버 API를 쉽게 다룰 수 있도록 코드 및 주석 상세히 작성.
- 회원가입 및 로그인 인증
